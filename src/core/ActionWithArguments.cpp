  ...
  for(unsigned i=0; i<c.size(); i++) {
    ...
    if(!c[i].compare(0,1,"(")) {
      ...
      if(!c[i].compare(l-1,1,")")) {
        // start regex parsing
#ifdef __PLUMED_HAS_REGEX
        bool found_something=false;
        // take the string enclosed in quotes and put in round brackets
        std::string myregex=c[i];
        std::vector<ActionWithValue*> all=plumed.getActionSet().select<ActionWithValue*>();
        if( all.empty() ) error("your input file is not telling plumed to calculate anything");
#if defined(__PLUMED_HAS_CXXREGEX) && defined(__PLUMED_HAS_CREGEX)
        bool use_cregex=std::getenv("PLUMED_CREGEX");
#elif defined(__PLUMED_HAS_CXXREGEX)
        bool use_cregex=false;
#elif defined(__PLUMED_HAS_CREGEX)
        bool use_cregex=true;
#else
#  error // one between __PLUMED_HAS_CXXREGEX and __PLUMED_HAS_CREGEX should be defined here
#endif
